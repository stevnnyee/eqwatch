...
def create_preference(body):
    ...
    with get_db() as conn:
        try:
            cursor = conn.cursor()
            cursor.execute(query("create_preference"), (body.user_id, body.min_magnitude, body.notify_email))
            conn.commit()
            return {"pref_id": cursor.lastrowid}
        except DatabaseError as e:
            if e.errno == 3819:
                raise HTTPException(status_code=422, detail="Magnitude is between -2 and 10.")
            raise
