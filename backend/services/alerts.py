...
def create_alert(body):
    ...
    with get_db() as conn:
        try:
            ...
            if e.errno == 1062:
                raise HTTPException(status_code=422, detail="user_id, eq_id pair already exists.")
            elif e.errno == 1452:
                raise HTTPException(status_code=422, detail="user_id and/or eq_id aren't valid.")
            raise
